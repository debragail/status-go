...
func (s *StatusAPISuite) TestAccessibleStatusAPIs() {
	...
	err := s.initTest(s.upstream, true)
	s.NoError(err)
	defer func() {
	...
	s.testAPIExported("status_login", false)
	s.testAPIExported("status_signup", false)

	// These status APIs should be available only for IPC
	s.testAPIPrivatelyExported("status_login", true)
	s.testAPIPrivatelyExported("status_signup", true)
}
...
func (s *GenericRPCApiSuite) testAPIPrivatelyExported(method string, expectExported bool) {
